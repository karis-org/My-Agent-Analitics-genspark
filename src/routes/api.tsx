...
4. **structure** (建物構造)
   - **必ず以下の4つのいずれかに正規化**: "RC造", "SRC造", "鉄骨造", "木造"
   - **RC造**: "鉄筋コンクリート造"、"RC"、"RC造"、"鉄筋コンクリート"、"ＲＣ造"、"ＲＣ" → "RC造"
   - **SRC造**: "鉄骨鉄筋コンクリート造"、"SRC"、"SRC造"、"鉄骨鉄筋コンクリート"、"ＳＲＣ造"、"ＳＲＣ" → "SRC造"
   - **鉄骨造**: "S造"、"鉄骨造"、"軽量鉄骨造"、"重量鉄骨造"、"鉄骨"、"Ｓ造" → "鉄骨造"
   - **木造**: "木造"、"W造"、"Ｗ造" → "木造"
   - **重要**: 部分一致で判定すること（例: "鉄筋コンクリート造3階建" → "RC造"）
   - 構造が不明な場合のみnullを返す
...
6. **age** (築年数)
   ...
   - **⚠️ 重要**: 以下の情報は築年数ではないので絶対に抽出しないこと:
     - 価格・売買金額: "900,000千円"、"4,500万円" → ❌ これは価格
     - 坪単価: "¥71,400/㎡"、"¥500,000/坪" → ❌ これは坪単価
     - 賃料・年間賃料: "31,728千円"、"¥317,280/月" → ❌ これは賃料
     - 数値が大きい（1000以上）場合は価格情報の可能性が高い → ❌ 築年数ではない
   - **築年数の特徴**:
     - 通常は0〜100の範囲内（ほとんどは0〜60年）
     - 「築XX年」と明記されている
     - 竣工年月から計算可能（例: 2020年築 → 5年）
   - 築年数が記載されている場合はそのまま
   ...
   - **築年数が不明な場合はnullを返す**（推測しない）
   - 例: "築5年" → 5
   - 例: "2020年3月築" → 5
   - 例: "平成26年5月築" → 11 (2014年築 = 2025-2014)
   - 例: "2026年7月竣工" → -1
   - 例: "¥71,400/㎡" → null（これは坪単価であり築年数ではない）
  ...
  "monthly_rent": 2644000
    ...
    if (parseErrors.length > 0) {
      ...
      return c.json({
        success: true,
        ...parsedData,
        warnings: parseErrors, // 警告情報をフロントエンドに通知
        hasWarnings: true,
        message: '一部のフィールドを正しく読み取れませんでした。手動で確認・修正してください。',
        confidence: 'medium' // エラーがある場合は信頼度を下げる
      });
    }
