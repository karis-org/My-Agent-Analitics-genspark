...
2. **price** (販売価格・売買価格)
   - **数値のみ**を抽出（単位を含めない）
   - 「万円」表記の場合は10,000倍して円に変換
   - 例: "4,500万円" → 45000000
   - 例: "5000万" → 50000000
   - 例: "3億2000万円" → 320000000
...
/**
 * Itandi BB Rental Market Analysis Endpoint
 * イタンジBB 賃貸相場調査エンドポイント
...
api.post('/itandi/rental-analysis', authMiddleware, async (c) => {
  try {
    const { env } = c;
    const body = await c.req.json();
    const { location, propertyType, area, radius } = body;

    if (!location) {
      return c.json({ error: 'Location is required' }, 400);
    }

    // Initialize Itandi client (will use demo data if API key not configured)
    const { ItandiClient } = await import('../lib/external-apis');
    const itandiClient = new ItandiClient(env.ITANDI_API_KEY || 'demo');

    // Get rental market analysis
    const analysis = await itandiClient.getRentalMarketAnalysis({
      location,
      propertyType,
    ...
    return c.json({
      success: true,
      mode: (!env.ITANDI_API_KEY || env.ITANDI_API_KEY === 'demo') ? 'demonstration' : 'full',
      ...analysis,
    });
  } catch (error: any) {
...
api.post('/itandi/rental-trend', authMiddleware, async (c) => {
  try {
    const { env } = c;
    const body = await c.req.json();
    const { location, propertyType, period } = body;

    if (!location) {
      return c.json({ error: 'Location is required' }, 400);
    }

    // Initialize Itandi client
    const { ItandiClient } = await import('../lib/external-apis');
    const itandiClient = new ItandiClient(env.ITANDI_API_KEY || 'demo');

    // Get rental trend data
    const trendData = await itandiClient.getRentalTrend({
      location,
      propertyType,
      period: period ? parseInt(period) : 12,
    });
    ...
    console.error('Rental trend error:', error);
    return c.json({
      error: '賃貸相場推移の取得に失敗しました',
      details: error.message,
    }, 500);
  }
});
